...
special_client.post(
    ...
    async (req: any, res) => {
      ...
      const tag = await Tag.findTagById(tagId);
      if (!tag) return res.status(400).json({ error: `Tag with id ${tagId} not found` });
      ...
      for (const file of files) {
        ...
        const normalized = original
          .replace(/[‐-‒–—−]/g, "-") // all dash variants -> "-"
          .replace(/[＿﹍﹎]/g, "_"); // underscore variants -> "_"
  
        // ✅ Accept "_" OR "," as separator between barcode and activation
        // Examples:
        // 1234567890123456_ABC-123.jpg
        // 1234567890123456,ABC-123.png
        const m = normalized.match(/^(\d{16})[_,](.+?)\.(jpg|jpeg|png)$/i);
        ...
        if (!m) {
          results.push({
            ...
            ok: false,
            error:
              "Filename must be 16DIGITBARCODE_ACTIVATIONCODE.jpg (separator '_' or ','; use normal '-')",
          });
          continue;
        }
        ...
        try {
          ...
          // S3 key: truly unique (timestamp + uuid)
          const safeOriginalName = file.originalname.replace(/\s+/g, "_");
          const key = `scans/${Date.now()}-${randomUUID()}-${barcodePart}-${safeOriginalName}`;
          ...
          if (
            ...
            msg = fieldList.length === 1 ? `${fieldList[0]} already scanned` : "already scanned";
            console.error("Duplicate scan error for file", name, e);
          } else {
            msg = e?.message || "Unknown error";
            console.error("Error handling file", name, e);
