...
export class User{
    ...
    constructor(_id:string, _email:string, _name:string, _role:string){
        isUndefined(
            {id:_id},
            {email:_email},
            {name: _name},
    ...
    public static findUserByEmail =  async (_email:string)=>{
        ...
        return new User(_user.id as string, _user.email as string, _user.name, _user.role as string);
    }
}
